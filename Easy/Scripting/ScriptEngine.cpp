...
module Easy.Script.ScriptingEngine;
...
namespace Easy {
    ...
    namespace Utils {
        ...
        ScriptFieldType MonoTypeToScriptFieldType(MonoType *monoType) {
            ...
            if (it == s_ScriptFieldTypeMap.end()) {
                EZ_CORE_ERROR("Unknown type: {}", typeName);
    ...
    struct ScriptEngineData {
        ...
        std::unordered_map<std::string, Arc<ScriptClass>> EntityClasses;
        std::unordered_map<UUID, Arc<ScriptInstance>> EntityInstances;
    ...
    void ScriptEngine::Init() {
        ...
        if (!status) {
            EZ_CORE_ERROR("[ScriptEngine] Could not load Hazel-ScriptCore assembly.");
            return;
        }
        ...
        if (!status) {
            EZ_CORE_ERROR("[ScriptEngine] Could not load app assembly.");
            return;
        }
    ...
    Arc<ScriptInstance> ScriptEngine::GetEntityScriptInstance(UUID entityID) {
    ...
    Arc<ScriptClass> ScriptEngine::GetEntityClass(const std::string &name) {
    ...
    std::unordered_map<std::string, Arc<ScriptClass>> ScriptEngine::GetEntityClasses() {
    ...
    void ScriptEngine::LoadAssemblyClasses() {
        ...
        for (int32_t i = 0; i < numTypes; i++) {
            ...
            EZ_CORE_WARN("{} has {} fields:", className, fieldCount);
            ...
            while (MonoClassField *field = mono_class_get_fields(monoClass, &iterator)) {
                ...
                if (flags & FIELD_ATTRIBUTE_PUBLIC) {
                    ...
                    EZ_CORE_WARN("  {} ({})", fieldName, Utils::ScriptFieldTypeToString(fieldType));
    ...
    MonoObject *ScriptEngine::GetManagedInstance(UUID uuid) {
        EZ_CORE_ASSERT(s_Data->EntityInstances.find(uuid) != s_Data->EntityInstances.end());
    ...
    ScriptInstance::ScriptInstance(Arc<ScriptClass> scriptClass, Entity entity)
