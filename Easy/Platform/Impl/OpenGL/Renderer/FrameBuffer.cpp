...
namespace Easy {
    ...
    namespace Utils {
        ...
        static GLenum EasyFBTextureFormatToGL(FramebufferTextureFormat format) {
            switch (format) {
    ...
    void OpenGLFramebuffer::Invalidate() {
        ...
        if (m_ColorAttachments.size() > 1) {
            EZ_CORE_ASSERT(m_ColorAttachments.size() <= 4);
            GLenum buffers[4] = {
        ...
        EZ_CORE_ASSERT(glCheckFramebufferStatus(GL_FRAMEBUFFER) == GL_FRAMEBUFFER_COMPLETE,
    ...
    void OpenGLFramebuffer::Resize(uint32_t width, uint32_t height) {
        if (width == 0 || height == 0 || width > s_MaxFramebufferSize || height > s_MaxFramebufferSize) {
            EZ_CORE_WARN("Attempted to rezize framebuffer to {0}, {1}", width, height);
            return;
        }
    ...
    int OpenGLFramebuffer::ReadPixel(uint32_t attachmentIndex, int x, int y) {
        EZ_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size());
    ...
    void OpenGLFramebuffer::ClearAttachment(uint32_t attachmentIndex, int value) {
        ...
        glClearTexImage(m_ColorAttachments[attachmentIndex], 0,
                        Utils::EasyFBTextureFormatToGL(spec.TextureFormat), GL_INT, &value);
    }
}
